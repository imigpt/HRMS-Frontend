...
import {
  ...
  Filter, Loader2, LayoutList, Columns3, FolderKanban, BarChart3, Timer,
  Users, Search,
} from 'lucide-react';
    ...
    <DashboardLayout>
      <div className="space-y-6 fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <Card className="glass-card card-hover"><CardContent className="p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-purple-500/20 flex items-center justify-center"><LayoutList className="h-5 w-5 text-purple-500" /></div><div><p className="text-2xl font-bold text-foreground">{stats.underReview}</p><p className="text-xs text-muted-foreground">Under Review</p></div></div></CardContent></Card>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="employees" className="mt-4">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                        ...
                        return (
                          <button key={emp._id} onClick={() => setEmployeeFilter(emp._id)}
                            ...
                            <Avatar className="h-7 w-7">
                              <AvatarFallback className="bg-gradient-to-br from-primary/20 to-primary/5 text-primary text-[9px] font-bold">{getInitials(emp.name)}</AvatarFallback>
                            </Avatar>
              ...
              <Card className="glass-card lg:col-span-3">
                <CardHeader>
                  <CardTitle className="text-sm">{employeeFilter === 'all' ? 'All Tasks' : `Tasks for ${employees.find(e => e._id === employeeFilter)?.name || 'Employee'}`}</CardTitle>
                </CardHeader>
                <CardContent>
          ...
          {/* Analytics */}
          <TabsContent value="analytics" className="mt-4"><TaskAnalytics /></TabsContent>
        </Tabs>
        ...
        {/* Task Detail Dialog */}
        <TaskDetailDialog task={selectedTask} open={isDetailOpen} onClose={() => setIsDetailOpen(false)} onTaskUpdated={fetchData} canReview={true} canUpload={false} canComment={true} showAssignee={true} userRole="admin" employees={employees} />
      </div>
    </DashboardLayout>
  );
};
